...
class PCIeSpoofGUI:
    ...
    def __init__(self, root):
        ...
        subtitle_label = ttk.Label(
            title_frame, 
            text="基于PCILeech-FPGA的设备伪装代码生成器", 
            font=("Arial", 12)
        )
        subtitle_label.pack()
    ...
    def create_tab_new_config(self):
        ...
        self.device_type_var.set("custom")
        
        type_combobox = ttk.Combobox(type_frame, textvariable=self.device_type_var)
        type_combobox['values'] = list(DEVICE_TYPES.keys())
        ...
        def on_preset_selected(event):
            ...
            if preset and preset != "无" and preset in PRESET_DEVICES:
                ...
                self.device_type_var.set(preset_data.get("type", "custom"))
            elif preset == "无":
    ...
    def create_tab_generation(self):
        ...
        ttk.Checkbutton(options_frame, text="生成配置空间文件", variable=self.gen_config_var).pack(anchor=tk.W)
        
        self.gen_bar_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(options_frame, text="生成BAR控制器代码", variable=self.gen_bar_var).pack(anchor=tk.W)
        
        self.gen_behavior_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(options_frame, text="生成行为模拟代码", variable=self.gen_behavior_var).pack(anchor=tk.W)
        
        self.gen_registers_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(options_frame, text="生成寄存器映射代码", variable=self.gen_registers_var).pack(anchor=tk.W)
        
        self.gen_interrupts_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(options_frame, text="生成中断处理代码", variable=self.gen_interrupts_var).pack(anchor=tk.W)
        
        self.gen_test_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(options_frame, text="生成测试脚本", variable=self.gen_test_var).pack(anchor=tk.W)
